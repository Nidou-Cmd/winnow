...
export function renderHtmlReport(audit) {
  ...
  const findingsHtml = audit.findings
    .map((f) => {
      ...
      const tfSnippet = `# Terraform / Pulumi Fix Generated by Winnow FinOps Engine
resource "datadog_metric_tag_configuration" "fix_${esc(f.title.toLowerCase().replace(/[^a-z0-9]/g, '_').slice(0, 20))}" {
  metric_name = "custom.metric.optimized"
  metric_type = "gauge"
  tags        = ["env", "service"] # Strips high-cardinality tags
  ...
  <div style="margin-top:14px;background:#050914;border:1px solid rgba(56,189,248,0.25);border-radius:10px;padding:12px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
      <span style="font-size:11px;font-weight:700;color:var(--blue);text-transform:uppercase;letter-spacing:.06em;">⚡ Code Terraform Prêt à Appliquer (1-Clic) :</span>
      <button onclick="navigator.clipboard.writeText(this.nextElementSibling.innerText);alert('✅ Code Terraform copié dans le presse-papier !');" style="background:rgba(56,189,248,0.15);color:var(--blue);border:1px solid rgba(56,189,248,0.3);padding:3px 8px;border-radius:6px;font-size:11px;font-weight:700;cursor:pointer;">Copier le Fix</button>
      <pre style="display:none;">${tfSnippet}</pre>
    </div>
    <pre style="font-family:'JetBrains Mono',monospace;font-size:11px;color:#38BDF8;background:#030712;padding:10px;border-radius:6px;overflow-x:auto;line-height:1.4;">${esc(tfSnippet)}</pre>
  </div>
